...
- Automatically track progress and tasks for an initiative.  
    - This approach updates initiative progress, syncs Azure DevOps work item details with Viva Goals initiative tasks, and, when used with the Viva Goals Azure DevOps extension, Viva Goals OKR alignment details surface directly in Azure DevOps linked work items.

## Outcomes (OKRs) vs Outputs (Initiatives)

One of the fundamental benefits of OKRs (objectives and key results) is distinguishing between outcomes and outputs. OKRs and key results are focused on driving impact (outcomes), while Viva Goals [initiatives](/viva/goals/projects) are focused on outputs – the work that is planned to achieve the key results. Both are important metrics to track, but they measure different things.
...
This method is the recommended approach for aligning work in Azure DevOps to OKRs in Viva Goals. In addition to tracking initiative progress based on Azure DevOps work items, this approach enables a cross-platform user experience. The Azure DevOps work item details sync to the Viva Goals initiative views and, when used with the Viva Goals Azure DevOps extension, the alignment of Azure DevOps work to Viva Goals OKRs is surfaced directly within the linked Azure DevOps work items.
...
1. Select the appropriate Azure DevOps connection. If you need to create a new Azure DevOps connection, you'll need to sign into Azure DevOps to create a data connection. A new connection is needed for each Azure DevOps project. After signing in:
    ...
    - Provide a connection name: You can include the Azure DevOps Organization and Project in the name for future reference.
...
The Azure DevOps icon now appears next to your initiative. The Azure DevOps work item details also now appear in your Viva Goals initiative views. Viva Goals automatically syncs the work item details and update the initiative progress once per hour based on the ADO field for the work item types selected within your shared query.

### If using the Connect to Work Items method

1. Select **work items** method.
1. Select the work item type to connect to – it's recommended to choose the parent work item type in your Azure DevOps hierarchy that contains the work being done. The integration will automatically include the child work items.
1. Search for and select the specific work item(s) of the chosen type.
1. Select the work item type you want to use for calculating progress of your initiative. You can choose to measure progress by a specific work item type (the type you directly connect to or any of its children types) or all work items (connected and children).  
1. Select the ADO field you want to use for tracking initiative progress.
1. Select **Next**.
1. Select **Save**.

The Azure DevOps icon now appears next to your initiative. The Azure DevOps work item details now also appear in your Viva Goals initiative views. Viva Goals automatically syncs the work item details and update this initiative progress once per hour based on the Azure DevOps field for the work item types selected.

Once you complete these steps, the Viva Goals initiative progress automatically updates regularly. In addition, the Azure DevOps work item details are shown in the Viva Goals initiative views, along with hyperlinks to the Azure DevOps work item details. If you enabled the Viva Goals Azure DevOps extension, the full alignment between OKRs and the Azure DevOps work items are accessible from a Viva Goals tab within the linked Azure DevOps work items.

## How to use Azure DevOps Integration with initiative KPIs

Use this method when you want to update the Viva Goals initiative progress with a single numeric value. This approach doesn't surface the Azure DevOps work items within Viva Goals initiative views. This method can't embed Viva Goals within the Azure DevOps work items.

1. Select **Add Initiative** in Viva Goals or edit an existing initiative.
1. Select **Outcome**.
1. Select **Add Metric** and fill in on the necessary details (name, type, units, starting, target values).
...
1. Select the appropriate Azure DevOps connection.  If you need to create a new Azure DevOps connection, you need to sign into Azure DevOps to create a data connection. A new connection is needed for each Azure DevOps project. After signing in:
    ...
    - Provide a Connection Name: you can include the Azure DevOps Organization and initiatives in the name for easy future reference.
    - Select the **Azure DevOps Organization**.
    - Select the **Azure DevOps Project**.
    - Select **Next**.
...
You should now see the Azure DevOps icon next to your initiative. Viva Goals automatically updates the progress once per hour.

### If using the Connect to Work Items method

1. Select **work items** method.
1. Select the ‘work item type’ to connect to – we recommend choosing the “parent” work item type in your Azure DevOps hierarchy that contains the work being done. The integration automatically includes the child work items. For example, connect to a feature that is the parent to the tasks versus connecting to each individual task.
1. Search for and select the specific work item(s) of the chosen type.
1. Select the work item type you want to use for calculating progress of your initiative. You can choose to measure progress by a specific work item type (the type you directly connect to or any of its children types) or all work items (connected and children).
1. Select the ADO field you want to use for tracking initiative progress.
1. Select **Next**.
...
You should now see the Azure DevOps icon next to your initiative. Viva Goals automatically updates this initiative once per hour.

## How to use Azure DevOps Integration with a Key Result

Use this method when you want to update the Viva Goals Key Result progress with a single numeric value. This approach doesn't surface the Azure DevOps work items within Viva Goals; nor does this method have the capability to embed Viva Goals within the Azure DevOps work items.
...
You should now see the Azure DevOps icon next to your key result. Viva Goals automatically updates the progress once per hour.
...
1. Search for and select the specific work item(s) of the chosen type.
1. Select the work item type you want to use for calculating progress of your key result. You can choose to measure progress by a specific work item type (the type you directly connect to or any of its children types) or all work items (connected and children).
1. Select the ADO field you want to use for tracking key result progress.
1. Select **Next**.
1. Select **Save**.

You should now see the Azure DevOps icon next to your Key Result. Viva Goals automatically updates this initiative once per hour based on the ADO field for the work item types selected.
...
If you encounter forbidden error messages when creating an Azure DevOps connection, you're likely encountering IP address constraints or other Conditional Access Policies. In this case, you need to verify if the Enable Microsoft Entra ID CAP validation policy is enabled on the Azure DevOps Organization. For more details, refer to the [Azure DevOps documentation.](/azure/devops/organizations/accounts/change-application-access-policies?view=azure-devops&preserve-view=true)
...
1. **When connecting my Viva Goals initiative tasks with Azure DevOps, why don’t I see all of the work items in my shared query and/or all of the child work items when using the “connect to work item” method?**
    ...
    However, we believe choosing “any” work item type is the exception, as this approach would expose an excessive amount of work item detail not necessary within Viva Goals UX. We assume most users choose to calculate progress by a specific work item type within the shared query or a child of the connected work items. Viva Goals will use that decision to limit what we expose in the Viva Goals UX.

    For example, assume you Azure DevOps work is tracking Features, User Stories and Tasks. Further assume, you choose to track Viva Goals initiative progress based on User Story completion. Viva Goals will only show the Features and User Stories within the Viva Goals initiative views. Users can quickly “double-click” into Azure DevOps for more details as needed.
